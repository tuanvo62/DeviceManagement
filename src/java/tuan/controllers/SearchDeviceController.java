...
public class SearchDeviceController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");
         try {
            String deviceName = request.getParameter("txtSearch");
            DeviceBLO blo = new DeviceBLO();
            List<Device> result = blo.findByLikeName(deviceName);
            request.setAttribute("DEVICEINFO", result);
        } catch (Exception e) {
            log("ERROR at SearchUserController: "+e.getMessage());
        }
        finally{
            request.getRequestDispatcher("adminDevice.jsp").forward(request, response);
        }
    }
